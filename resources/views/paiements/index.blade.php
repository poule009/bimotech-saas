...
<div class="space-y-4 md:space-y-6">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('admin.paiements.export-csv', request()->query()) }}"
               class="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-bimo-navy/15
                      rounded-[10px] font-body font-medium text-sm text-bimo-navy/60
                      hover:text-bimo-navy hover:border-bimo-navy/30 transition-all duration-150"
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            <div class="font-body font-medium text-[9.5px] uppercase tracking-widest text-bimo-navy/50 mb-1">Net propriétaires</div>
            ...
            <div class="font-body text-[10.5px] text-bimo-navy/40 mt-1.5">FCFA à reverser</div>
        </div>
        ...
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            <div class="font-body font-medium text-[9.5px] uppercase tracking-widest text-bimo-navy/50 mb-1">Paiements validés</div>
            <div class="font-display font-extrabold text-xl text-bimo-navy leading-none">
                {{ $stats['nb_payes'] }}
            </div>
    ...
    <form method="GET" class="flex flex-wrap gap-2 items-center">
        <input type="month" name="mois" value="{{ request('mois') }}"
               ...
               class="px-3 py-2 bg-white border border-bimo-navy/15 rounded-[9px]
                      font-body text-sm text-bimo-navy focus:outline-none focus:border-bimo-gold
    ...
    <div class="bg-white rounded-[14px] border border-bimo-navy/10 py-16 px-6 text-center">
        ...
        <div class="font-display font-bold text-base text-bimo-navy mb-2">Aucun paiement trouvé</div>
        ...
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 overflow-hidden">
            ...
            <div class="flex items-center justify-between px-4 py-3 bg-bimo-bg2 border-b border-bimo-navy/[5%]">
                <div class="flex items-center gap-2">
                    <span class="font-body text-[10px] text-bimo-navy/40 uppercase tracking-widest">{{ $p->reference_paiement }}</span>
                    <button onclick="copyRef('{{ $p->reference_paiement }}', this)"
                            class="w-5 h-5 flex items-center justify-center border border-bimo-navy/15 rounded-[4px]
                                   text-bimo-navy/30 hover:text-bimo-navy hover:border-bimo-navy/30 transition-all duration-150">
            ...
            <div class="px-4 py-3.5 space-y-2.5">
                ...
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <div class="font-body font-semibold text-sm text-bimo-navy">{{ $p->contrat?->bien?->reference ?? '—' }}</div>
                        ...
                        <a href="{{ route('admin.bailleurs.show', $proprio->id) }}"
                           class="font-body text-[10px] text-bimo-gold hover:text-bimo-navy transition-colors duration-150">
                            ↗ {{ $proprio->name }}
                        </a>
                        @endif
                    </div>
                ...
                <div class="grid grid-cols-3 gap-2 pt-2 border-t border-bimo-navy/[5%]">
                    <div>
                        <div class="font-body text-[9.5px] uppercase tracking-widest text-bimo-navy/40 mb-0.5">Commission</div>
                        <div class="font-body font-medium text-xs text-bimo-navy/70">{{ number_format($p->commission_ttc ?? 0, 0, ',', ' ') }} F</div>
                    </div>
                    <div>
                        <div class="font-body text-[9.5px] uppercase tracking-widest text-bimo-navy/40 mb-0.5">Net proprio</div>
                        <div class="font-body font-medium text-xs text-bimo-navy">{{ number_format($p->net_a_verser_proprietaire ?? $p->net_proprietaire ?? 0, 0, ',', ' ') }} F</div>
                    </div>
                    <div>
                        <div class="font-body text-[9.5px] uppercase tracking-widest text-bimo-navy/40 mb-0.5">Mode</div>
                        <div class="font-body text-xs text-bimo-navy/60">{{ \App\Http\Controllers\PaiementController::MODES_PAIEMENT[$p->mode_paiement] ?? $p->mode_paiement }}</div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-2 pt-1">
                    <a href="{{ route('admin.paiements.show', $p) }}"
                       class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-bimo-bg border border-bimo-navy/10
                              rounded-[7px] font-body text-xs text-bimo-navy/60 hover:text-bimo-navy transition-all duration-150">
            ...
            <table class="w-full text-sm" data-sort-id="paiements-table">
                <thead>
                    <tr class="border-b border-bimo-navy/[5%] bg-bimo-bg">
                        ...
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/40 cursor-pointer hover:text-bimo-gold transition-colors duration-150" data-sort="1">Bien / Locataire</th>
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/40 cursor-pointer hover:text-bimo-gold transition-colors duration-150" data-sort="2" data-sort-type="date">Période</th>
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/40 cursor-pointer hover:text-bimo-gold transition-colors duration-150" data-sort="3" data-sort-type="date">Date</th>
                        <th class="px-5 py-3 text-right font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/40 cursor-pointer hover:text-bimo-gold transition-colors duration-150" data-sort="4" data-sort-type="num">Montant</th>
                        <th class="px-5 py-3 text-right font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/40 cursor-pointer hover:text-bimo-gold transition-colors duration-150" data-sort="5" data-sort-type="num">Commission</th>
                        <th class="px-5 py-3 text-right font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/40 cursor-pointer hover:text-bimo-gold transition-colors duration-150" data-sort="6" data-sort-type="num">Net proprio</th>
                    ...
                    <tr class="hover:bg-bimo-bg transition-colors duration-100">
                        ...
                        <td class="px-5 py-3.5 whitespace-nowrap">
                            <div class="flex items-center gap-1.5">
                                <span class="font-body text-[11px] text-bimo-navy/40 uppercase tracking-widest">{{ $p->reference_paiement }}</span>
                                <button onclick="copyRef('{{ $p->reference_paiement }}', this)"
