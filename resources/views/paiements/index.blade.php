...
<div style="padding:24px 32px 48px">
    ...
    <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:22px">
        ...
        <a href="{{ route('admin.paiements.create') }}" class="btn-primary">
    ...
    <div class="kpi-row">
        <div class="kpi-mini gold">
            ...
            <div class="kpi-val">{{ number_format($totalEncaisse, 0, ',', ' ') }}<span class="kpi-u">F</span></div>
            <div class="kpi-s">{{ $paiements->count() }} paiements affichés</div>
        </div>
        <div class="kpi-mini green">
            <div class="kpi-lbl">Net propriétaires</div>
            <div class="kpi-val">{{ number_format($totalNet, 0, ',', ' ') }}<span class="kpi-u">F</span></div>
            <div class="kpi-s">Après commission agence</div>
        </div>
        <div class="kpi-mini blue">
            <div class="kpi-lbl">Commission agence</div>
            <div class="kpi-val">{{ number_format($totalCommission, 0, ',', ' ') }}<span class="kpi-u">F</span></div>
            <div class="kpi-s">TTC · TVA 18% incluse</div>
        </div>
        <div class="kpi-mini red">
            <div class="kpi-lbl">Paiements validés</div>
            <div class="kpi-val">{{ $nbValides }}<span class="kpi-u">/ {{ $paiements->count() }}</span></div>
    ...
    <form method="GET" action="{{ route('admin.paiements.index') }}">
        <div class="filter-bar">
            ...
            <div style="position:relative;flex:1;min-width:180px">
                ...
                <input type="text" name="q" value="{{ request('q') }}" placeholder="Référence, locataire, bien…" class="filter-input" style="padding-left:34px;width:100%">
            </div>
            ...
            <select name="mode" class="filter-select">
                ...
                @foreach(\App\Models\Paiement::MODES_PAIEMENT as $key => $label)
                    <option value="{{ $key }}" {{ request('mode') === $key ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
            </select>

            {{-- Mois --}}
            <input type="month" name="periode" value="{{ request('periode') }}" class="filter-input" style="width:150px">

            <button type="submit" class="filter-btn">Filtrer</button>

            @if(request()->hasAny(['q','statut','mode','periode']))
                <a href="{{ route('admin.paiements.index') }}" class="filter-reset">
            ...
            {{-- Export --}}
            <a href="{{ route('admin.rapports.financier.export-pdf') }}" class="filter-reset">
    ...
    <div class="table-card">
        <div class="table-header">
            <div>
                ...
                <div class="table-count">{{ $paiements->total() }} paiement(s) au total · Page {{ $paiements->currentPage() }} / {{ $paiements->lastPage() }}</div>
            </div>
            <div style="display:flex;align-items:center;gap:8px">
                ...
                <select class="filter-select" style="font-size:12px;padding:6px 10px" onchange="window.location=this.value">
                    <option value="{{ request()->fullUrlWithQuery(['sort'=>'date']) }}" {{ request('sort','date') === 'date' ? 'selected':'' }}>Date</option>
                    <option value="{{ request()->fullUrlWithQuery(['sort'=>'montant']) }}" {{ request('sort') === 'montant' ? 'selected':'' }}>Montant</option>
                    <option value="{{ request()->fullUrlWithQuery(['sort'=>'periode']) }}" {{ request('sort') === 'periode' ? 'selected':'' }}>Période</option>
                </select>
            </div>
        </div>
            ...
            <table class="dt">
                ...
                <tbody>
                    ...
                    <tr>
                        {{-- Référence --}}
                        <td>
                            <span class="ref-tag">{{ $p->reference_paiement }}</span>
                        </td>

                        {{-- Bien & Locataire --}}
                        <td>
                            <div class="bien-ref">{{ $p->contrat?->bien?->reference ?? '—' }}</div>
                            <div class="bien-sub">{{ $p->contrat?->locataire?->name ?? '—' }}</div>
                            @if($p->contrat?->bien?->ville)
                                <div class="bien-sub">{{ $p->contrat->bien->ville }}</div>
                            @endif
                        </td>
                        ...
                        {{-- Date --}}
                        <td>
                            <div style="font-size:13px;color:#374151">{{ \Carbon\Carbon::parse($p->date_paiement)->format('d/m/Y') }}</div>
                        </td>
                        ...
                        {{-- Montant --}}
                        <td class="td-r">
                            <div class="montant">{{ number_format($p->montant_encaisse, 0, ',', ' ') }} F</div>
                            @if($p->commission_ttc)
                                <div class="commission">Com. {{ number_format($p->commission_ttc, 0, ',', ' ') }} F</div>
                            @endif
                        </td>

                        {{-- Net proprio --}}
                        <td class="td-r">
                            <div class="net">{{ number_format($p->net_proprietaire, 0, ',', ' ') }} F</div>
                        </td>
                        ...
                        <td class="td-c">
                            <div style="display:flex;align-items:center;justify-content:center;gap:6px">
                                {{-- Voir --}}
                                <a href="{{ route('admin.paiements.show', $p) }}" class="act-btn" title="Voir le détail">
                                ...
                                {{-- PDF --}}
                                @if($p->statut === 'valide')
                                    <a href="{{ route('admin.paiements.pdf', $p) }}" target="_blank" class="act-btn" title="Télécharger quittance PDF">
                            ...
                            <div class="empty-state">
                                ...
                                <div class="empty-sub">
                                    @if(request()->hasAny(['q','statut','mode','periode']))
                                        ...
                                        <a href="{{ route('admin.paiements.index') }}" style="color:#c9a84c;font-weight:500">Effacer les filtres</a>
                                    @else
        ...
        <div class="pagination-wrap">
            ...
            <div class="pagination-links">
                ...
                @else
                    <a href="{{ $paiements->previousPageUrl() }}" class="page-btn">
                        <svg style="width:14px;height:14px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg>
                    </a>
                @endif
                ...
                @foreach($paiements->getUrlRange(max(1,$paiements->currentPage()-2), min($paiements->lastPage(),$paiements->currentPage()+2)) as $page => $url)
                    <a href="{{ $url }}" class="page-btn {{ $page === $paiements->currentPage() ? 'active' : '' }}">
                        {{ $page }}
                    </a>
                @endforeach
                ...
                @if($paiements->hasMorePages())
                    <a href="{{ $paiements->nextPageUrl() }}" class="page-btn">
