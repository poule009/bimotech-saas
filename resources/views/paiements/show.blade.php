<x-app-layout>
    <x-slot name="header">Détails du Paiement — {{ $paiement->reference_paiement }}</x-slot>
        ...
        <div style="display:flex; align-items:center; gap:20px;">
            ...
            <div>
                <div style="display:flex; align-items:center; gap:12px;">
                    ...
                    @endphp
                    <span style="background: {{ $statusStyles['bg'] }}; color: {{ $statusStyles['text'] }}; padding: 6px 14px; border-radius: 10px; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px;">
                        {{ $statusStyles['label'] }}
                    </span>
                </div>
                <p style="font-size:13px; color:#b58c5a; font-weight:600; text-transform: uppercase; letter-spacing: 1px; margin-top: 4px;">
                    Échéance de {{ \Carbon\Carbon::parse($paiement->periode)->translatedFormat('F Y') }}
                </p>
            </div>
        </div>
    ...
    <div style="max-width:720px;">
        ...
        {{-- Parties concernées (Look "Carte Privée") --}}
            ...
            <div class="card-body" style="padding: 32px;">
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;" id="parties-grid">
                    ...
                    <div style="padding:18px; background:#fcfaf5; border:1px solid rgba(181, 140, 90, 0.1); border-radius:16px;">
                        ...
                        <div style="font-weight:800; font-size:15px; color:var(--agency);">{{ $paiement->contrat->bien->proprietaire->name }}</div>
                        <div style="font-size:12px; color:#718096; margin-top:4px; font-weight: 500;">Tel: {{ $paiement->contrat->bien->proprietaire->telephone ?? '—' }}</div>
                    </div>
                    ...
                    <div style="padding:18px; background:#fcfaf5; border:1px solid rgba(181, 140, 90, 0.1); border-radius:16px;">
                        ...
                        <div style="font-weight:800; font-size:15px; color:var(--agency);">{{ $paiement->contrat->locataire->name }}</div>
                        <div style="font-size:12px; color:#718096; margin-top:4px; font-weight: 500;">Tel: {{ $paiement->contrat->locataire->telephone ?? '—' }}</div>
                    </div>
                    ...
                    <div style="padding:18px; background:#fcfaf5; border:1px solid rgba(181, 140, 90, 0.1); border-radius:16px;">
                        ...
                        <div style="font-weight:800; font-size:15px; color:var(--agency);">{{ $paiement->contrat->bien->reference }}</div>
                        <div style="font-size:12px; color:#718096; margin-top:4px; font-weight: 500;">{{ $paiement->contrat->bien->adresse }}, {{ $paiement->contrat->bien->ville }}</div>
                    </div>
                    ...
                    <div style="padding:18px; background:#1a202c; border-radius:16px; color: white;">
                        ...
                        <div style="font-weight:800; font-size:15px; color:#b58c5a;">{{ ucfirst(str_replace('_', ' ', $paiement->mode_paiement)) }}</div>
                        <div style="font-size:12px; color:#a0aec0; margin-top:4px; font-weight: 500;">
                            Perçu le {{ \Carbon\Carbon::parse($paiement->date_paiement)->format('d/m/Y') }}
                        </div>
                    </div>
                ...
                @if($paiement->notes)
                    <div style="margin-top:20px; padding:18px; background:#f7f4ec; border-radius:16px; border-left:4px solid #b58c5a;">
                        ...
                        <div style="font-size:14px; color:var(--agency); line-height: 1.5; font-weight: 500;">{{ $paiement->notes }}</div>
                    </div>
                @endif
            </div>
        </div>

        {{-- Détail des montants (Style "Reçu de Banque") --}}
        <div class="card section-gap" style="background: white; border: none; border-radius: 28px; box-shadow: 0 15px 40px rgba(0,0,0,0.04); overflow: hidden; margin-bottom: 30px;">
            ...
            <div class="card-body" style="padding: 32px;">
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; padding:16px 0; border-bottom:1px solid #f7f4ec;">
                    ...
                    <span style="font-weight:900; font-size:18px; color:var(--agency);" class="text-money">
                        {{ number_format($paiement->montant_encaisse, 0, ',', ' ') }} <small style="font-size:12px; font-weight:600; opacity:0.6;">F</small>
                    </span>
                </div>

                <div style="display:flex; justify-content:space-between; align-items:center; padding:16px 0; border-bottom:1px solid #f7f4ec;">
                    <span style="font-size:14px; color:#718096; font-weight: 600;">
                        Honoraires de Gestion HT ({{ $paiement->taux_commission_applique }}%)
                    </span>
                    <span style="color:#d97706; font-weight:700; font-size:15px;" class="text-money">
                        − {{ number_format($paiement->commission_agence, 0, ',', ' ') }} F
                    </span>
                </div>
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid #f7f4ec;">
                    ...
                    <span style="font-size:13px; color:#a0aec0; font-weight:700;" class="text-money">
                        − {{ number_format($paiement->tva_commission, 0, ',', ' ') }} F
                    </span>
                </div>
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; padding:16px 0; border-bottom:1px solid #f7f4ec;">
                    ...
                    <span style="color:#b45309; font-weight:900; font-size:16px;" class="text-money">
                        − {{ number_format($paiement->commission_ttc, 0, ',', ' ') }} F
                    </span>
                </div>
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; padding:24px; background: linear-gradient(135deg, #16a34a, #15803d); border-radius:20px; margin-top:20px; color:white; box-shadow: 0 10px 20px rgba(22, 163, 74, 0.2);">
                    ...
                    <span style="font-weight:900; font-size:26px; letter-spacing:-1px;" class="text-money">
                        {{ number_format($paiement->net_proprietaire, 0, ',', ' ') }} <small style="font-size:14px;">F</small>
                    </span>
                </div>
