...
<div class="space-y-4 md:space-y-6">
    ...
    <div class="flex items-start justify-between gap-4">
        ...
        @else
        <div class="flex-shrink-0 inline-flex items-center gap-2 bg-bimo-navy/5 text-bimo-navy/40
        ...
        <button @click="open = !open"
            ...
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-[9px] bg-bimo-navy/5 flex items-center justify-center flex-shrink-0">
                    <svg class="w-4 h-4 text-bimo-navy/50" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                        <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
                    </svg>
                </div>
                <span class="font-display font-semibold text-sm text-bimo-navy">Comment fonctionne la gestion d'équipe ?</span>
            </div>
            <svg class="w-4 h-4 text-bimo-navy/30 flex-shrink-0 transition-transform duration-150"
        ...
        <div x-show="open" x-transition:enter="transition duration-150"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div class="bg-bimo-bg rounded-[10px] p-4">
                    <p class="font-display font-semibold text-xs text-bimo-navy uppercase tracking-widest mb-3">
                    ...
                    <ul class="space-y-1.5">
                        ...
                        <li class="flex items-start gap-2">
                            ...
                            <span class="font-body text-xs text-bimo-navy/70">{{ $item }}</span>
                        </li>
                        @endforeach
                    </ul>
                </div>
            ...
            <p class="font-body text-xs text-bimo-navy/40 leading-relaxed">
    ...
    <div class="bg-bimo-gold/[8%] border border-bimo-gold/25 rounded-[14px] px-5 py-4 flex items-start gap-3">
        ...
        <div>
            <p class="font-body font-medium text-sm text-bimo-navy">Limite de {{ $limiteMax }} collaborateurs atteinte</p>
            <p class="font-body text-xs text-bimo-navy/50 mt-0.5">
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-bimo-navy/10 bg-bimo-bg2">
                    <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/50">Nom</th>
                    <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/50">Email</th>
                    <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/50">Rôle</th>
                    <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/50">Membre depuis</th>
                ...
                <tr class="hover:bg-bimo-bg transition-colors duration-100">
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="font-body font-medium text-sm text-bimo-navy">{{ $collab->name }}</p>
                                @if($collab->telephone)
                                <p class="font-body text-xs text-bimo-navy/40">{{ $collab->telephone }}</p>
                                @endif
                            </div>
                        </div>
                    </td>
                    <td class="px-5 py-4 font-body text-sm text-bimo-navy/70">{{ $collab->email }}</td>
                    <td class="px-5 py-4">
                        @if($collab->is_owner)
                        ...
                        @else
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-body font-medium
                                     bg-bimo-navy/10 border border-bimo-navy/15 text-bimo-navy/70">
                            Collaborateur
                        </span>
                        @endif
                    </td>
                    <td class="px-5 py-4 font-body text-sm text-bimo-navy/50">
        ...
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            <div class="flex items-center justify-between mb-3">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="font-body font-medium text-sm text-bimo-navy">{{ $collab->name }}</p>
                        <p class="font-body text-xs text-bimo-navy/40">{{ $collab->email }}</p>
                    </div>
                </div>
                @if($collab->is_owner)
                ...
                @else
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-body font-medium
                             bg-bimo-navy/10 text-bimo-navy/60">Collaborateur</span>
                @endif
            </div>
            @if(!$collab->is_owner && $collab->id !== auth()->id())
            <form method="POST" action="{{ route('admin.equipe.destroy', $collab) }}"
