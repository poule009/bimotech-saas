...
<div style="padding:24px 32px 48px">
    ...
    <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:22px">
        <div>
            ...
            <p style="font-size:13px;color:#6b7280;margin-top:3px">
                {{ $periode->translatedFormat('F Y') }} · {{ $stats['nb_impayes'] }} impayé(s) sur {{ $stats['nb_impayes'] + $stats['nb_payes'] }} contrats actifs
            </p>
        </div>
        ...
        <div class="month-nav">
            ...
            @endif
            @if(!$isCurrentMonth)
                <a href="{{ route('admin.impayes.index') }}" style="padding:6px 12px;font-size:12px;color:#6b7280;border:1px solid #e5e7eb;border-radius:7px;text-decoration:none;background:#fff">Mois actuel</a>
            @endif
        </div>
    </div>
    ...
    <div class="kpi-row">
        <div class="kpi-mini red">
            ...
            <div class="kpi-val red">{{ $stats['nb_impayes'] }}<span class="kpi-u">contrats</span></div>
            <div class="kpi-s">{{ $stats['nb_impayes'] > 0 ? 'Action requise' : 'Aucun impayé' }}</div>
        </div>
        <div class="kpi-mini orange">
            <div class="kpi-lbl">Montant dû</div>
            <div class="kpi-val" style="color:#d97706;font-size:18px">{{ number_format($stats['montant_du'], 0, ',', ' ') }}<span class="kpi-u">F</span></div>
            <div class="kpi-s">À recouvrer ce mois</div>
        </div>
        <div class="kpi-mini green">
            <div class="kpi-lbl">Payés</div>
            <div class="kpi-val green">{{ $stats['nb_payes'] }}<span class="kpi-u">contrats</span></div>
            <div class="kpi-s">Loyers encaissés</div>
        </div>
        <div class="kpi-mini blue">
            <div class="kpi-lbl">Taux de recouvrement</div>
            <div class="kpi-val" style="color:#1d4ed8">{{ $stats['taux_recouvrement'] }}<span class="kpi-u">%</span></div>
            <div class="kpi-s">{{ $stats['nb_payes'] }} / {{ $stats['nb_impayes'] + $stats['nb_payes'] }} contrats</div>
        </div>
    </div>

    {{-- BARRE RECOUVREMENT GLOBALE --}}
    <div class="recouvrement-bar">
        <div>
            <div class="rec-label">Impayés</div>
            <div class="rec-val red">{{ number_format($stats['montant_du'], 0, ',', ' ') }} F</div>
        </div>
        <div class="rec-progress-wrap">
            ...
            <div class="rec-progress-bg">
                <div class="rec-progress-fill" style="width:{{ $taux }}%;background:{{ $progressColor }}"></div>
            </div>
            <div class="rec-progress-label">
                <span>0%</span>
                <span style="color:{{ $progressColor }};font-weight:600">{{ $taux }}% recouvré</span>
                <span>100%</span>
            </div>
        </div>
        <div>
            ...
            @endphp
            <div class="rec-val green">{{ number_format($totalRecouvre, 0, ',', ' ') }} F</div>
        </div>
    </div>
        ...
        <div class="filter-bar">
            ...
            @if($stats['nb_impayes'] > 0)
                <span style="font-size:12px;color:#dc2626;font-weight:600;display:flex;align-items:center;gap:5px">
                    ...
                    {{ $stats['nb_impayes'] }} relance(s) en attente
                </span>
            @endif
        </div>
    </form>
    ...
    <div class="table-card danger">
        <div class="table-hd danger">
            <div>
                ...
                <div style="font-size:11px;color:#dc2626;margin-top:2px">{{ $periode->translatedFormat('F Y') }}</div>
            </div>
            <div class="table-count red">{{ $stats['nb_impayes'] }} contrat(s) · {{ number_format($stats['montant_du'], 0, ',', ' ') }} F dûs</div>
        </div>
        ...
        @if($impayes->isEmpty())
            <div class="empty-state">
                ...
                <div class="empty-sub">Tous les loyers de {{ $periode->translatedFormat('F Y') }} ont été encaissés.</div>
            </div>
        @else
                ...
                <table class="dt">
                    ...
                    <tbody>
                        ...
                        <tr class="{{ $rowClass }}">
                            {{-- Bien --}}
                            <td>
                                <div class="bien-ref">{{ $item['contrat']->bien->reference }}</div>
                                <div class="bien-sub">{{ $item['contrat']->bien->adresse }}, {{ $item['contrat']->bien->ville }}</div>
                            </td>

                            {{-- Locataire --}}
                            <td>
                                <div class="loc-name">{{ $item['contrat']->locataire->name }}</div>
                                <div class="loc-phone">{{ $item['contrat']->locataire->email }}</div>
                            </td>

                            {{-- Téléphone --}}
                            <td>
                                @if($item['contrat']->locataire->telephone)
                                    <a href="tel:{{ $item['contrat']->locataire->telephone }}"
                            ...
                            <td class="td-c">
                                @if($jours > 0)
                                    <span class="retard-badge {{ $retardClass }}">
                                        @if($retardClass === 'critique')
                            ...
                            {{-- Montant --}}
                            <td class="td-r">
                                <div class="montant">{{ number_format($item['montant_du'], 0, ',', ' ') }} F</div>
                                ...
                                <div class="act-group" style="justify-content:center">
                                    ...
                                    <form method="POST" action="{{ route('admin.impayes.relance', $item['contrat']) }}">
                                        ...
                                        <button type="submit" class="act-btn relance"
                                                onclick="return confirm('Envoyer une relance email à {{ $item['contrat']->locataire->name }} ?')">
                                    ...
                                    @endphp
                                    <a href="{{ $waUrl }}" target="_blank" class="act-btn whatsapp" title="Envoyer un message WhatsApp">
    ...
    <div class="table-card">
        <div class="table-hd success">
            <div>
                ...
                <div style="font-size:11px;color:#16a34a;margin-top:2px">{{ $periode->translatedFormat('F Y') }}</div>
            </div>
            <div class="table-count green">{{ $stats['nb_payes'] }} paiement(s) validé(s)</div>
        </div>
        ...
        @if($payes->isEmpty())
            <div class="empty-state">
                ...
                <div class="empty-sub">Aucun loyer n'a encore été encaissé pour {{ $periode->translatedFormat('F Y') }}.</div>
            </div>
        @else
            <div style="overflow-x:auto">
                <table class="dt">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <div class="bien-ref">{{ $item['contrat']->bien->reference }}</div>
                                <div class="bien-sub">{{ $item['contrat']->bien->ville }}</div>
                            </td>
                            <td>
                                <div class="loc-name">{{ $item['contrat']->locataire->name }}</div>
                                <div class="loc-phone">{{ $item['contrat']->locataire->telephone ?? $item['contrat']->locataire->email }}</div>
                            </td>
                            <td>
                                <span style="font-size:12px;color:#6b7280">
                                    {{ \App\Models\Paiement::MODES_PAIEMENT[$item['paiement']->mode_paiement] ?? $item['paiement']->mode_paiement }}
                                </span>
                            </td>
                            <td>
                            ...
                            <td class="td-r">
                                <div class="montant green">{{ number_format($item['paiement']->montant_encaisse, 0, ',', ' ') }} F</div>
                            </td>
                            <td class="td-r">
                            ...
                            <td class="td-c">
                                @if($item['paiement']->statut === 'valide')
                                    <span class="badge g"><span class="bdot"></span>Validé</span>
                                @else
                                    <span class="badge o"><span class="bdot"></span>{{ ucfirst($item['paiement']->statut) }}</span>
                                @endif
                            </td>
                            <td class="td-c">
                                @if($item['paiement']->statut === 'valide')
