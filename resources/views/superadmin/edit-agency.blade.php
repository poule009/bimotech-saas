...
<div class="max-w-2xl">
    ...
    <nav class="flex items-center gap-2 font-body text-sm text-bimo-navy/40 mb-5">
        ...
        <a href="{{ route('superadmin.agencies.show', $agency) }}" class="hover:text-bimo-navy transition-colors duration-150">{{ $agency->name }}</a>
        <svg class="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg>
        <span class="text-bimo-navy font-semibold">Modifier</span>
    </nav>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $agency->name) }}" required
                               class="w-full px-4 py-3 rounded-[10px] bg-white border font-body text-sm text-bimo-navy focus:outline-none focus:ring-2 transition-all duration-150 @error('name') border-bimo-red focus:border-bimo-red focus:ring-bimo-red/15 @else border-bimo-navy/20 focus:border-bimo-gold focus:ring-bimo-gold/15 @enderror">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="email" id="email" name="email" value="{{ old('email', $agency->email) }}" required
                               class="w-full px-4 py-3 rounded-[10px] bg-white border font-body text-sm text-bimo-navy focus:outline-none focus:ring-2 transition-all duration-150 @error('email') border-bimo-red focus:border-bimo-red focus:ring-bimo-red/15 @else border-bimo-navy/20 focus:border-bimo-gold focus:ring-bimo-gold/15 @enderror">
                        @error('email')<p class="font-body text-xs text-bimo-red">{{ $message }}</p>@enderror
                    </div>
                    <div class="space-y-1.5">
                        <label class="block font-body font-medium text-sm text-bimo-navy" for="telephone">Téléphone <span class="font-light text-bimo-navy/40">(optionnel)</span></label>
                    ...
                    <div class="space-y-1.5">
                        <label class="block font-body font-medium text-sm text-bimo-navy" for="adresse">Adresse <span class="font-light text-bimo-navy/40">(optionnel)</span></label>
                        <input type="text" id="adresse" name="adresse" value="{{ old('adresse', $agency->adresse) }}" placeholder="Rue 10, Dakar"
                               class="w-full px-4 py-3 rounded-[10px] bg-white border border-bimo-navy/20 font-body text-sm text-bimo-navy placeholder:text-bimo-navy/35 focus:outline-none focus:border-bimo-gold focus:ring-2 focus:ring-bimo-gold/15 transition-all duration-150">
                    </div>
                    <div class="space-y-1.5">
                        <label class="block font-body font-medium text-sm text-bimo-navy" for="taux_tva">Taux TVA (%) <span class="font-light text-bimo-navy/40">(optionnel)</span></label>
