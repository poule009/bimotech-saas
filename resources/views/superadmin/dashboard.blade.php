...
<div class="space-y-4 md:space-y-5">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            <div class="font-body font-medium text-[9.5px] uppercase tracking-widest text-bimo-navy/50 mb-1.5">{{ $lbl }}</div>
            <div class="font-display font-extrabold text-2xl text-bimo-navy leading-none">{{ $val }}</div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            <div class="font-body font-medium text-[9.5px] uppercase tracking-widest text-bimo-navy/50 mb-1.5">Commissions plateforme</div>
            <div class="font-display font-extrabold text-lg text-bimo-navy leading-none">{{ number_format($stats['total_commissions'],0,',','') }}</div>
            <div class="font-body text-[10.5px] text-bimo-navy/40 mt-1.5">FCFA TTC cumulés</div>
        </div>
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            <div class="font-body font-medium text-[9.5px] uppercase tracking-widest text-bimo-navy/50 mb-1.5">Revenus abonnements</div>
            <div class="font-display font-extrabold text-lg text-bimo-navy leading-none">{{ number_format($stats['revenus_abonnements'],0,',','') }}</div>
            <div class="font-body text-[10.5px] text-bimo-navy/40 mt-1.5">FCFA encaissés</div>
        </div>
        <div class="bg-white rounded-[14px] border {{ $stats['nb_expires'] > 0 ? 'border-bimo-red/20' : 'border-bimo-navy/10' }} p-4">
            <div class="font-body font-medium text-[9.5px] uppercase tracking-widest {{ $stats['nb_expires'] > 0 ? 'text-bimo-red/70' : 'text-bimo-navy/50' }} mb-1.5">Expirés</div>
            <div class="font-display font-extrabold text-2xl {{ $stats['nb_expires'] > 0 ? 'text-bimo-red' : 'text-bimo-navy' }} leading-none">{{ $stats['nb_expires'] }}</div>
            <div class="font-body text-[10.5px] {{ $stats['nb_expires'] > 0 ? 'text-bimo-red/60' : 'text-bimo-navy/40' }} mt-1.5">{{ $stats['nb_expires'] > 0 ? 'À relancer' : 'Aucun' }}</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[14px] border border-bimo-navy/10 overflow-hidden">
        <div class="flex items-center justify-between px-5 py-4 border-b border-bimo-navy/[5%] bg-bimo-bg2 flex-wrap gap-2">
            <div>
                <span class="font-display font-bold text-sm text-bimo-navy">Évolution sur 12 mois</span>
                <div class="font-body text-xs text-bimo-navy/40 mt-0.5">Revenus abonnements &amp; nouvelles agences</div>
            </div>
            <div class="flex items-center gap-4 font-body text-xs text-bimo-navy/40">
    ...
    <div class="bg-white rounded-[14px] border border-bimo-navy/10 overflow-hidden">
        <div class="flex items-center justify-between px-5 py-4 border-b border-bimo-navy/[5%] bg-bimo-bg2">
            <div>
                <span class="font-display font-bold text-sm text-bimo-navy">Toutes les agences</span>
                <div class="font-body text-xs text-bimo-navy/40 mt-0.5"><span id="sa-count">{{ $agences->count() }}</span> agence(s)</div>
            </div>
        ...
        <div class="flex items-center gap-2 px-5 py-3 border-b border-bimo-navy/[5%] flex-wrap">
            <div class="relative flex-1 min-w-[180px] max-w-xs">
                ...
                <input type="text" id="sa-search" placeholder="Rechercher une agence…"
                       class="w-full pl-9 pr-3 py-2 border border-bimo-navy/15 rounded-[8px] font-body text-sm text-bimo-navy bg-bimo-bg focus:outline-none focus:border-bimo-gold focus:bg-white transition-all duration-150"
                       oninput="saFilter()">
            </div>
            @foreach([['tous','Tous'],['essai','Essai'],['actif','Actifs'],['expire','Expirés'],['suspendu','Suspendus']] as [$val,$lbl])
            <button class="filter-tab px-3.5 py-1.5 rounded-full font-body font-medium text-xs border border-bimo-navy/15 text-bimo-navy/50 hover:border-bimo-gold hover:text-bimo-gold transition-all duration-150 {{ $val === 'tous' ? 'bg-bimo-navy text-white border-bimo-navy' : '' }}"
        ...
        <div class="md:hidden divide-y divide-bimo-navy/[5%]" id="sa-mobile-list">
            ...
            <div class="px-4 py-3.5 {{ !$agence->actif ? 'opacity-50' : '' }}" data-name="{{ strtolower($agence->name) }} {{ strtolower($agence->email) }}" data-statut="{{ $filterStatut }}">
                <div class="flex items-center justify-between gap-2 mb-1">
                    <a href="{{ route('superadmin.agencies.show', $agence) }}" class="font-body font-semibold text-sm text-bimo-navy">{{ $agence->name }}</a>
                    @if($agence->actif)
                ...
                <div class="font-body text-xs text-bimo-navy/40">{{ $agence->email }}</div>
            </div>
            @empty
            <div class="px-5 py-10 text-center font-body text-sm text-bimo-navy/30">Aucune agence.</div>
            @endforelse
        </div>
            ...
            <table class="w-full text-sm" id="sa-table">
                <thead>
                    <tr class="border-b border-bimo-navy/[5%] bg-bimo-bg">
                        ...
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/40">Abonnement</th>
                        ...
                        <th class="px-5 py-3 text-center font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/40">Contrats</th>
                        <th class="px-5 py-3 text-right font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/40">Loyers</th>
                        <th class="px-5 py-3 text-right font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/40">Commissions</th>
                        <th class="px-5 py-3 text-center font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/40">Depuis</th>
                ...
                <tbody class="divide-y divide-bimo-navy/[5%]">
                    ...
                    <tr class="hover:bg-bimo-bg transition-colors duration-100 {{ !$agence->actif ? 'opacity-50' : '' }}"
                        ...
                        <td class="px-5 py-3.5">
                            <a href="{{ route('superadmin.agencies.show', $agence) }}" class="font-body font-semibold text-sm text-bimo-navy hover:text-bimo-gold transition-colors duration-150">{{ $agence->name }}</a>
                            <div class="font-body text-[11px] text-bimo-navy/40 mt-0.5">{{ $agence->email }}</div>
                        </td>
                        <td class="px-5 py-3.5">
                            @if($sub)
                                @if($sub->statut === 'essai')
                                ...
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-body font-semibold bg-bimo-navy/10 border border-bimo-navy/15 text-bimo-navy/70"><span class="w-1.5 h-1.5 rounded-full bg-bimo-navy/50 mr-1"></span>{{ ucfirst($sub->plan ?? 'Actif') }}</span>
                                ...
                                @else
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-body font-medium bg-bimo-navy/[5%] text-bimo-navy/50">{{ $sub->statut }}</span>
                                @endif
                                @if($jours !== null)
                                <div class="font-body text-[10px] mt-1 {{ $jours <= 7 ? 'text-bimo-red' : ($jours <= 30 ? 'text-bimo-gold' : 'text-bimo-navy/30') }}">
                                    {{ $jours >= 0 ? $jours.' j restants' : abs($jours).' j dépassés' }}
                                </div>
                                @endif
                            @else
                                <span class="font-body text-xs text-bimo-navy/30">Aucun</span>
                            @endif
                        </td>
                        <td class="px-5 py-3.5 text-center">
                            @if($agence->actif)
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-body font-semibold bg-bimo-navy/10 border border-bimo-navy/15 text-bimo-navy/70"><span class="w-1.5 h-1.5 rounded-full bg-bimo-navy/50 mr-1"></span>Active</span>
                            @else
                        ...
                        <td class="px-5 py-3.5 text-center font-body text-sm text-bimo-navy/60">{{ $agence->biens_count }}</td>
                        <td class="px-5 py-3.5 text-center font-body text-sm text-bimo-navy/60">{{ $agence->contrats_count }}</td>
                        <td class="px-5 py-3.5 text-right font-display font-semibold text-sm text-bimo-navy/70">{{ number_format($agence->total_loyers,0,',','') }}</td>
                        ...
                        <td class="px-5 py-3.5 text-center font-body text-xs text-bimo-navy/40">{{ $agence->created_at->format('d/m/Y') }}</td>
                        <td class="px-5 py-3.5 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('superadmin.agencies.show', $agence) }}"
                                   class="inline-flex items-center px-3 py-1.5 border border-bimo-navy/15 rounded-[7px] font-body text-xs text-bimo-navy/60 hover:border-bimo-navy/30 hover:text-bimo-navy transition-all duration-150">
                                    Détail
                                </a>
                                <form method="POST" action="{{ route('superadmin.agencies.toggle', $agence) }}"
                                    ...
                                    @csrf @method('PATCH')
                                    <button type="submit"
                                            class="inline-flex items-center px-3 py-1.5 rounded-[7px] font-body text-xs border transition-all duration-150 {{ $agence->actif ? 'border-bimo-red/20 bg-bimo-red/[5%] text-bimo-red hover:bg-bimo-red/10' : 'border-bimo-navy/15 text-bimo-navy/60 hover:border-bimo-navy/30 hover:text-bimo-navy' }}">
                    ...
                    @empty
                    <tr>
                        <td colspan="9" class="px-5 py-12 text-center font-body text-sm text-bimo-navy/30">
                            Aucune agence.
                            <a href="{{ route('superadmin.agencies.create') }}" class="text-bimo-gold hover:text-bimo-navy ml-1 transition-colors duration-150">Créer la première →</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div id="sa-empty-filter" class="hidden px-5 py-12 text-center font-body text-sm text-bimo-navy/30">
...
function saSetFilter(btn, filter) {
    ...
    document.querySelectorAll('.filter-tab').forEach(function(b) {
        ...
        b.classList.add('text-bimo-navy/50','border-bimo-navy/15');
    });
    btn.classList.add('bg-bimo-navy','text-white','border-bimo-navy');
    btn.classList.remove('text-bimo-navy/50','border-bimo-navy/15');
    saFilter();
}
function saFilter() {
