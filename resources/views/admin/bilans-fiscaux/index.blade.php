...
<div class="space-y-4 md:space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
            <h1 class="font-display font-extrabold text-xl md:text-2xl text-bimo-text tracking-tight">Bilans fiscaux propriétaires</h1>
            <p class="font-body text-sm text-bimo-text/50 mt-1">Abattement 30% · IRPP · CFPB · TVA collectée · BRS — Année {{ $annee }}</p>
        </div>
        <div class="flex-shrink-0 bg-bimo-gold/[6%] border border-bimo-gold/20 rounded-[10px] px-4 py-3 text-right">
            ...
            <div class="font-body text-xs text-bimo-text/50 mt-0.5">À vérifier avec un comptable / DGI</div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            <div class="font-body font-medium text-[9.5px] uppercase tracking-widest text-bimo-text/50 mb-1.5">TVA loyer collectée</div>
            <div class="font-display font-extrabold text-xl text-bimo-gold leading-none">{{ number_format($totalTvaCol,0,',','') }}<span class="font-body text-xs text-bimo-text/40 ml-1">F</span></div>
            <div class="font-body text-[10.5px] text-bimo-text/40 mt-1.5">À reverser à la DGI</div>
        </div>
        <div class="bg-white rounded-[14px] border {{ $totalBrs > 0 ? 'border-bimo-red/20' : 'border-bimo-navy/10' }} p-4">
            <div class="font-body font-medium text-[9.5px] uppercase tracking-widest {{ $totalBrs > 0 ? 'text-bimo-red/70' : 'text-bimo-text/50' }} mb-1.5">BRS total retenu</div>
            <div class="font-display font-extrabold text-xl {{ $totalBrs > 0 ? 'text-bimo-red' : 'text-bimo-text' }} leading-none">{{ number_format($totalBrs,0,',','') }}<span class="font-body text-xs text-bimo-text/40 ml-1">F</span></div>
            <div class="font-body text-[10.5px] text-bimo-text/40 mt-1.5">Par les locataires entreprises</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[14px] border border-bimo-navy/10 overflow-hidden">
        <div class="flex items-center justify-between px-5 py-4 border-b border-bimo-navy/[5%] bg-bimo-bg2">
            <span class="font-display font-bold text-sm text-bimo-text">Propriétaires — Bilan {{ $annee }}</span>
            <span class="font-body text-xs text-bimo-text/40">{{ $proprietaires->count() }} propriétaire(s)</span>
        </div>
        {{-- Mobile --}}
        <div class="md:hidden divide-y divide-bimo-navy/[5%]">
            ...
            <div class="px-4 py-3.5">
                <div class="flex items-center justify-between gap-2 mb-1">
                    <div>
                        <div class="font-body font-semibold text-sm text-bimo-text">{{ $proprio->name }}</div>
                        <div class="font-body text-xs text-bimo-text/40">{{ $proprio->email }}</div>
                    </div>
                    @if($bilan)
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-body font-semibold bg-bimo-navy/10 border border-bimo-navy/15 text-bimo-text/70">Calculé</span>
                    @else
                ...
                <div class="flex items-center gap-2 mt-2">
                    <form method="POST" action="{{ route('admin.bilans-fiscaux.calculate', $proprio) }}">
                        ...
                        <button type="submit" class="inline-flex items-center gap-1 px-3 py-1.5 border border-bimo-navy/15 rounded-[7px] font-body text-xs text-bimo-text/60 hover:border-bimo-gold hover:text-bimo-gold transition-all duration-150 cursor-pointer">
                    ...
                    @if($bilan)
                    <a href="{{ route('admin.bilans-fiscaux.show', [$proprio, 'annee' => $annee]) }}" class="inline-flex items-center gap-1 px-3 py-1.5 border border-bimo-navy/15 rounded-[7px] font-body text-xs text-bimo-text/60 hover:border-bimo-gold hover:text-bimo-gold transition-all duration-150">
                        Voir
                    </a>
                    @endif
                </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-bimo-navy/[5%] bg-bimo-bg">
                        ...
                        <th class="px-5 py-3 text-right font-body font-medium text-[10px] uppercase tracking-widest text-bimo-text/40">Base imposable (70%)</th>
                        ...
                        <th class="px-5 py-3 text-right font-body font-medium text-[10px] uppercase tracking-widest text-bimo-text/40">TVA collectée</th>
                        <th class="px-5 py-3 text-right font-body font-medium text-[10px] uppercase tracking-widest text-bimo-text/40">BRS retenu</th>
                ...
                <tbody class="divide-y divide-bimo-navy/[5%]">
                    ...
                    <tr class="hover:bg-bimo-bg transition-colors duration-100">
                        ...
                        <td class="px-5 py-3.5 text-right font-display font-semibold text-sm text-bimo-text/70">
                            @if($bilan) {{ number_format($bilan->revenus_bruts_loyers,0,',','') }} F @else <span class="text-bimo-text/20">—</span> @endif
                        </td>
                        ...
                        <td class="px-5 py-3.5 text-right font-display font-bold text-sm text-bimo-red">
                            @if($bilan) {{ number_format($bilan->irpp_estime,0,',','') }} F @else <span class="text-bimo-text/20">—</span> @endif
                        </td>
                        <td class="px-5 py-3.5 text-center">
                            ...
                            @endphp
                            @if($rc === 'cgf')
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-body font-semibold bg-bimo-navy/10 border border-bimo-navy/15 text-bimo-text/70">CGF</span>
                            <div class="font-body text-[10px] text-bimo-text/30 mt-0.5">Art. 80 CGI SN</div>
                            @elseif($rc === 'irpp')
                            ...
                            <div class="font-body text-[10px] text-bimo-text/30 mt-0.5">Art. 65 CGI SN</div>
                            @else
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-body font-medium bg-bimo-navy/[5%] text-bimo-text/40">N/A &gt;30M</span>
                            @endif
                            @else <span class="text-bimo-text/20">—</span> @endif
                        </td>
                        <td class="px-5 py-3.5 text-right font-display font-semibold text-sm text-bimo-gold">
                            ...
                            @elseif($bilan) <span class="text-bimo-text/20">—</span>
                            @else <span class="text-bimo-text/20">—</span> @endif
                        </td>
                        <td class="px-5 py-3.5 text-right font-display font-bold text-sm text-bimo-red">
                        ...
                        <td class="px-5 py-3.5 text-center">
                            @if($bilan)
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-body font-semibold bg-bimo-navy/10 border border-bimo-navy/15 text-bimo-text/70">Calculé</span>
                            <div class="font-body text-[10px] text-bimo-text/30 mt-0.5">{{ $bilan->calcule_le->format('d/m/Y') }}</div>
                            @else
