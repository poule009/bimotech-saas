...
<div class="space-y-4 md:space-y-6">
    ...
    <div class="flex items-start justify-between gap-4">
        <div>
            <h1 class="font-display font-extrabold text-xl md:text-2xl text-bimo-navy tracking-tight leading-tight">Comptabilité</h1>
            <p class="font-body text-sm text-bimo-navy/50 mt-1">{{ now()->locale('fr')->translatedFormat('F Y') }}</p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            <p class="font-body font-medium text-[9.5px] uppercase tracking-widest text-bimo-navy/50 mb-2">Charges du mois</p>
            <p class="font-display font-extrabold text-xl text-bimo-navy">{{ number_format($resultat['charges_total'], 0, ',', ' ') }}</p>
            <p class="font-body font-light text-[10.5px] text-bimo-navy/40 mt-1">FCFA</p>
        </div>
        <div class="bg-white rounded-[14px] border {{ $resultat['resultat_net'] >= 0 ? 'border-bimo-navy/10' : 'border-bimo-red/20' }} p-4">
            <p class="font-body font-medium text-[9.5px] uppercase tracking-widest text-bimo-navy/50 mb-2">Résultat net</p>
            <p class="font-display font-extrabold text-xl {{ $resultat['resultat_net'] >= 0 ? 'text-bimo-navy' : 'text-bimo-red' }}">
                {{ ($resultat['resultat_net'] >= 0 ? '+' : '') . number_format($resultat['resultat_net'], 0, ',', ' ') }}
            </p>
            <p class="font-body font-light text-[10.5px] text-bimo-navy/40 mt-1">FCFA</p>
        </div>
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            <p class="font-body font-medium text-[9.5px] uppercase tracking-widest text-bimo-navy/50 mb-2">Dû aux propriétaires</p>
            <p class="font-display font-extrabold text-xl text-bimo-navy">{{ number_format($soldesMandants->sum('solde'), 0, ',', ' ') }}</p>
            <p class="font-body font-light text-[10.5px] text-bimo-navy/40 mt-1">FCFA total</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-5">
        <div class="flex items-center justify-between mb-4">
            <h2 class="font-display font-bold text-sm text-bimo-navy">Évolution 6 derniers mois</h2>
            <div class="flex items-center gap-4 font-body text-[10px] uppercase tracking-widest text-bimo-navy/40">
        ...
        <div class="grid grid-cols-6 gap-2 items-end h-32">
            ...
            <div class="flex flex-col items-center gap-1">
                ...
                <span class="font-body text-[9px] uppercase tracking-widest text-bimo-navy/30">{{ $m['mois'] }}</span>
            </div>
            @endforeach
        </div>
    </div>
        ...
        <a href="{{ route('admin.comptabilite.compte-resultat') }}"
           class="bg-white rounded-[14px] border border-bimo-navy/10 p-5 flex items-center gap-4 hover:border-bimo-navy/25 transition-colors duration-150">
            <div class="w-10 h-10 rounded-[10px] bg-bimo-navy/[5%] flex items-center justify-center flex-shrink-0">
                <svg class="w-5 h-5 text-bimo-navy/60" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
            </div>
            <div>
                <p class="font-display font-bold text-sm text-bimo-navy">Compte de résultat</p>
                <p class="font-body text-xs text-bimo-navy/40 mt-0.5">Revenus, charges, résultat net</p>
            </div>
        </a>
        <a href="{{ route('admin.charges-agence.index') }}"
           class="bg-white rounded-[14px] border border-bimo-navy/10 p-5 flex items-center gap-4 hover:border-bimo-navy/25 transition-colors duration-150">
            <div class="w-10 h-10 rounded-[10px] bg-bimo-navy/[5%] flex items-center justify-center flex-shrink-0">
                <svg class="w-5 h-5 text-bimo-navy/60" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
            </div>
            <div>
                <p class="font-display font-bold text-sm text-bimo-navy">Charges agence</p>
                <p class="font-body text-xs text-bimo-navy/40 mt-0.5">Salaires, loyer, téléphone...</p>
            </div>
        </a>
        <a href="{{ route('admin.reversements.index') }}"
           class="bg-white rounded-[14px] border border-bimo-navy/10 p-5 flex items-center gap-4 hover:border-bimo-navy/25 transition-colors duration-150">
            <div class="w-10 h-10 rounded-[10px] bg-bimo-navy/[5%] flex items-center justify-center flex-shrink-0">
                <svg class="w-5 h-5 text-bimo-navy/60" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>
            </div>
            <div>
                <p class="font-display font-bold text-sm text-bimo-navy">Reversements</p>
                <p class="font-body text-xs text-bimo-navy/40 mt-0.5">Comptes mandants propriétaires</p>
            </div>
        </a>
    </div>
    ...
    <div class="bg-white rounded-[14px] border border-bimo-navy/10 overflow-hidden">
        <div class="px-5 py-4 border-b border-bimo-navy/[5%] bg-bimo-bg2 flex items-center justify-between">
            <span class="font-display font-bold text-sm text-bimo-navy">Soldes en attente de reversement</span>
            ...
            <div class="px-5 py-3.5 flex items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('admin.reversements.compte-mandant', $prop) }}"
                       class="inline-flex items-center px-3 py-1 border border-bimo-navy/15 rounded-[7px] font-body text-xs text-bimo-navy/60 hover:border-bimo-navy/30 hover:text-bimo-navy transition-all duration-150">
                        Voir
                    </a>
                </div>
            </div>
