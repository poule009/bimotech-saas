...
<div class="max-w-2xl mx-auto">
    <form method="POST" action="{{ route('admin.charges-agence.update', $charge) }}" class="space-y-6">
        ...
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="font-display font-extrabold text-xl text-bimo-navy tracking-tight">Modifier la charge</h1>
        </div>
        ...
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-5 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <input id="date_charge" name="date_charge" type="date" value="{{ old('date_charge', $charge->date_charge->format('Y-m-d')) }}"
                           class="w-full px-4 py-3 rounded-[10px] bg-white border font-body text-sm text-bimo-navy focus:outline-none focus:ring-2 transition-all duration-150 border-bimo-navy/20 focus:border-bimo-gold focus:ring-bimo-gold/15">
                </div>
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <select id="categorie" name="categorie" class="w-full px-4 py-3 rounded-[10px] bg-white border border-bimo-navy/20 font-body text-sm text-bimo-navy focus:outline-none focus:border-bimo-gold focus:ring-2 focus:ring-bimo-gold/15 transition-all duration-150 appearance-none cursor-pointer">
            ...
            <div class="space-y-1.5">
                <label for="notes" class="block font-body font-medium text-sm text-bimo-navy">Notes</label>
                <textarea id="notes" name="notes" rows="3"
                          class="w-full px-4 py-3 rounded-[10px] bg-white border border-bimo-navy/20 font-body text-sm text-bimo-navy focus:outline-none focus:border-bimo-gold focus:ring-2 focus:ring-bimo-gold/15 transition-all duration-150 resize-none">{{ old('notes', $charge->notes) }}</textarea>
            </div>
        </div>
