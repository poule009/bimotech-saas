...
<div class="space-y-4 md:space-y-6">
    ...
    <div class="flex items-start justify-between gap-4">
        <div>
            <h1 class="font-display font-extrabold text-xl md:text-2xl text-bimo-text tracking-tight leading-tight">Charges agence</h1>
            <p class="font-body text-sm text-bimo-text/50 mt-1">Dépenses opérationnelles de l'agence</p>
        </div>
    ...
    <form method="GET" class="flex flex-wrap items-center gap-2">
        <input type="month" name="mois" value="{{ $mois }}" onchange="this.form.submit()"
               class="px-3 py-2 rounded-[8px] bg-white border border-bimo-navy/20 font-body text-sm text-bimo-text focus:outline-none focus:border-bimo-gold focus:ring-2 focus:ring-bimo-gold/15 transition-all duration-150">
        <select name="categorie" onchange="this.form.submit()" class="px-3 py-2 rounded-[8px] bg-white border border-bimo-navy/20 font-body text-sm text-bimo-text focus:outline-none focus:border-bimo-gold focus:ring-2 focus:ring-bimo-gold/15 transition-all duration-150 appearance-none cursor-pointer">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div class="col-span-2 bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            <p class="font-body font-medium text-[9.5px] uppercase tracking-widest text-bimo-text/50 mb-2">Total {{ $mois ? \Carbon\Carbon::createFromFormat('Y-m', $mois)->locale('fr')->translatedFormat('F Y') : '' }}</p>
            <p class="font-display font-extrabold text-2xl text-bimo-text">{{ number_format($total, 0, ',', ' ') }} <span class="text-sm font-body font-normal text-bimo-text/40">FCFA</span></p>
        </div>
    ...
    <div class="md:hidden space-y-3">
        ...
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            ...
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <span class="font-body text-[10px] text-bimo-text/30">{{ $charge->date_charge->format('d/m/Y') }}</span>
            </div>
        ...
        @empty
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-8 text-center font-body text-sm text-bimo-text/30">Aucune charge pour cette période.</div>
        @endforelse
    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-bimo-navy/[5%] bg-bimo-bg2">
                        ...
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-text/40">Libellé</th>
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-text/40">Catégorie</th>
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-text/40">Prestataire</th>
                ...
                <tbody class="divide-y divide-bimo-navy/[5%]">
                    ...
                    <tr class="hover:bg-bimo-bg transition-colors duration-100">
                        <td class="px-5 py-3.5 font-body text-xs text-bimo-text/50">{{ $charge->date_charge->format('d/m/Y') }}</td>
                        <td class="px-5 py-3.5 font-body font-medium text-sm text-bimo-text">{{ $charge->libelle }}</td>
                    ...
                    @empty
                    <tr><td colspan="6" class="px-5 py-10 text-center font-body text-sm text-bimo-text/30">Aucune charge pour cette période.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
