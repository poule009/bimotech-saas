...
<div class="max-w-2xl mx-auto">
    <form method="POST" action="{{ route('admin.reversements.store') }}" class="space-y-6">
        ...
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="font-display font-extrabold text-xl text-bimo-navy tracking-tight">Nouveau reversement</h1>
        </div>
        ...
        <div class="bg-bimo-gold/[8%] border border-bimo-gold/25 rounded-[14px] p-4 flex items-center justify-between">
            <div>
                <p class="font-body font-medium text-sm text-bimo-navy">Solde dû à {{ $proprietaireSelectionne?->name }}</p>
                <p class="font-body text-xs text-bimo-navy/50 mt-0.5">Net à reverser calculé</p>
            </div>
        ...
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-5 space-y-4">
            ...
            <div class="space-y-1.5">
                <label for="proprietaire_id" class="block font-body font-medium text-sm text-bimo-navy">Propriétaire <span class="text-bimo-red">*</span></label>
                <select id="proprietaire_id" name="proprietaire_id" onchange="this.form.submit()"
                        class="w-full px-4 py-3 rounded-[10px] bg-white border border-bimo-navy/20 font-body text-sm text-bimo-navy focus:outline-none focus:border-bimo-gold focus:ring-2 focus:ring-bimo-gold/15 transition-all duration-150 appearance-none cursor-pointer @error('proprietaire_id') border-bimo-red @enderror">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input id="montant" name="montant" type="number" min="1" step="1"
                           ...
                           class="w-full px-4 py-3 rounded-[10px] bg-white border font-body text-sm text-bimo-navy focus:outline-none focus:ring-2 transition-all duration-150 @error('montant') border-bimo-red focus:border-bimo-red focus:ring-bimo-red/15 @else border-bimo-navy/20 focus:border-bimo-gold focus:ring-bimo-gold/15 @enderror">
                    @error('montant')<p class="font-body text-xs text-bimo-red">{{ $message }}</p>@enderror
                </div>
                <div class="space-y-1.5">
                    <label for="date_reversement" class="block font-body font-medium text-sm text-bimo-navy">Date <span class="text-bimo-red">*</span></label>
            ...
            <div class="space-y-1.5">
                <label for="mode_paiement" class="block font-body font-medium text-sm text-bimo-navy">Mode de paiement <span class="text-bimo-red">*</span></label>
                <select id="mode_paiement" name="mode_paiement" class="w-full px-4 py-3 rounded-[10px] bg-white border border-bimo-navy/20 font-body text-sm text-bimo-navy focus:outline-none focus:border-bimo-gold focus:ring-2 focus:ring-bimo-gold/15 transition-all duration-150 appearance-none cursor-pointer @error('mode_paiement') border-bimo-red @enderror">
            ...
            <div class="space-y-1.5">
                <label for="reference" class="block font-body font-medium text-sm text-bimo-navy">Référence <span class="font-light text-bimo-navy/40 ml-1">(optionnel)</span></label>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    <label for="periode_debut" class="block font-body font-medium text-sm text-bimo-navy">Période du <span class="font-light text-bimo-navy/40 ml-1">(optionnel)</span></label>
                    <input id="periode_debut" name="periode_debut" type="month" value="{{ old('periode_debut') }}"
                           class="w-full px-4 py-3 rounded-[10px] bg-white border border-bimo-navy/20 font-body text-sm text-bimo-navy focus:outline-none focus:border-bimo-gold focus:ring-2 focus:ring-bimo-gold/15 transition-all duration-150">
                </div>
                <div class="space-y-1.5">
                    <label for="periode_fin" class="block font-body font-medium text-sm text-bimo-navy">Au</label>
