...
<div class="space-y-4">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <a href="{{ route('admin.etats-trimestriels.index') }}"
               class="inline-flex items-center gap-1.5 font-body text-sm text-bimo-text/40 hover:text-bimo-text transition-colors duration-150 mb-2">
                <svg class="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg>
                Retour
            </a>
            <h1 class="font-display font-extrabold text-xl text-bimo-text tracking-tight">État trimestriel BRS — T{{ $trimestre }} {{ $annee }}</h1>
        ...
        <div class="flex items-center gap-2 flex-shrink-0">
            ...
            <a href="{{ route('admin.etats-trimestriels.csv', [$annee, $trimestre]) }}"
               class="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-bimo-navy/15 text-bimo-text/60 font-body text-sm rounded-[10px] hover:border-bimo-navy/30 hover:text-bimo-text transition-all duration-150">
    ...
    <div class="bg-white rounded-[14px] border border-bimo-navy/10 overflow-hidden">
        <div class="flex items-center justify-between px-5 py-4 border-b border-bimo-navy/[5%] bg-bimo-bg2">
            <span class="font-display font-bold text-sm text-bimo-text">Détail par bailleur — T{{ $trimestre }} {{ $annee }}</span>
            <span class="font-body text-xs text-bimo-text/40">{{ $lignes->count() }} bailleur(s)</span>
        </div>

        @if($lignes->isEmpty())
        <div class="px-5 py-10 text-center font-body text-sm text-bimo-text/30">Aucun paiement avec BRS retenu sur ce trimestre.</div>
        @else
        ...
        <div class="md:hidden divide-y divide-bimo-navy/[5%]">
            ...
            <div class="px-4 py-3.5">
                <div class="flex items-start justify-between gap-2 mb-1.5">
                    <div>
                        <div class="font-body font-semibold text-sm text-bimo-text">{{ $ligne['nom_complet'] }}</div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-bimo-navy/[5%] bg-bimo-bg">
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-text/40">Bailleur</th>
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-text/40">NINEA / Pièce d'identité</th>
                        ...
                        <th class="px-5 py-3 text-right font-body font-medium text-[10px] uppercase tracking-widest text-bimo-text/40">Loyers nets versés</th>
                ...
                <tbody class="divide-y divide-bimo-navy/[5%]">
                    ...
                    <tr class="hover:bg-bimo-bg transition-colors duration-100">
                        <td class="px-5 py-3.5">
                            <div class="font-body font-semibold text-sm text-bimo-text">{{ $ligne['nom_complet'] }}</div>
                            @if($ligne['email'])<div class="font-body text-xs text-bimo-text/50">{{ $ligne['email'] }}</div>@endif
                            @if($ligne['telephone'])<div class="font-body text-xs text-bimo-text/30">{{ $ligne['telephone'] }}</div>@endif
                        </td>
                        <td class="px-5 py-3.5">
                            @if($ligne['ninea'])
                            <div class="font-body font-semibold text-sm text-bimo-text" style="font-family:monospace">{{ $ligne['ninea'] }}</div>
                            @else
                            ...
                            @if($ligne['cni'])<div class="font-body text-xs text-bimo-text/50 mt-1">CNI : {{ $ligne['cni'] }}</div>@endif
                            @if($ligne['date_naissance'])<div class="font-body text-xs text-bimo-text/50">Né(e) le : {{ \Carbon\Carbon::parse($ligne['date_naissance'])->format('d/m/Y') }}</div>@endif
                            <div class="font-body text-[10px] text-bimo-gold/70 italic mt-0.5">⚠ Mettre à jour avant dépôt DGID</div>
                            @endif
                        </td>
                        <td class="px-5 py-3.5 font-body text-sm text-bimo-text/60">{{ $ligne['adresse'] ?: '—' }}</td>
                        <td class="px-5 py-3.5">
    ...
    <div class="flex items-start gap-2 bg-bimo-navy/[4%] border border-bimo-navy/10 rounded-[10px] px-4 py-3">
        ...
        <p class="font-body text-xs text-bimo-text/50 leading-relaxed">
