...
<div class="space-y-4 md:space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
            ...
            <p class="font-body text-sm text-bimo-text/50 mt-1">{{ $stats['total'] }} propriétaire(s) enregistré(s)</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            <div class="font-body font-medium text-[9.5px] uppercase tracking-widest text-bimo-text/50 mb-1">Biens gérés</div>
            <div class="font-display font-extrabold text-2xl text-bimo-text leading-none">{{ $stats['total_biens'] }}</div>
            <div class="font-body text-[10.5px] text-bimo-text/40 mt-1.5">{{ $stats['biens_loues'] }} loué(s) · {{ $stats['total_biens'] - $stats['biens_loues'] }} disponible(s)</div>
        </div>
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            ...
            <div class="font-display font-extrabold text-2xl leading-none {{ $taux >= 80 ? 'text-bimo-gold' : 'text-bimo-text' }}">{{ $taux }}%</div>
            <div class="font-body text-[10.5px] text-bimo-text/40 mt-1.5">Biens avec locataire actif</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[14px] border border-bimo-navy/10 overflow-hidden">
        <div class="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-bimo-navy/[5%] bg-bimo-bg2">
            <span class="font-display font-bold text-sm text-bimo-text">Liste des propriétaires</span>
            <div class="relative">
        ...
        <div class="md:hidden divide-y divide-bimo-navy/[5%]" id="mobile-list">
            ...
            <div class="px-5 py-4" data-search="{{ strtolower($user->name . ' ' . $user->email . ' ' . $user->telephone) }}">
                <div class="flex items-center gap-3 mb-3">
                    <div class="w-10 h-10 rounded-[10px] flex items-center justify-center font-display font-bold text-sm text-bimo-text flex-shrink-0"
                    ...
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-body font-medium
                                 {{ $user->biens_count > 0 ? 'bg-bimo-gold/10 border border-bimo-gold/20 text-bimo-gold' : 'bg-bimo-navy/10 border border-bimo-navy/15 text-bimo-text/60' }}">
            ...
            <table class="w-full text-sm" id="dt-proprio">
                <thead>
                    <tr class="border-b border-bimo-navy/[5%] bg-bimo-bg">
                        ...
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widests text-bimo-text/40">Ville</th>
                        <th class="px-5 py-3 text-center font-body font-medium text-[10px] uppercase tracking-widest text-bimo-text/40">Biens</th>
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-text/40">Paiement</th>
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-text/40">NINEA</th>
                ...
                <tbody class="divide-y divide-bimo-navy/[5%]">
                    ...
                    <tr class="hover:bg-bimo-bg transition-colors duration-100">
                        <td class="px-5 py-3.5">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-[9px] flex items-center justify-center font-display font-bold text-sm text-bimo-text flex-shrink-0"
                                     style="background: var(--ac)">
                                    {{ mb_strtoupper(mb_substr($user->name, 0, 2)) }}
                                </div>
                                <div>
                                    <div class="font-body font-semibold text-sm text-bimo-text">{{ $user->name }}</div>
                                    <div class="font-body text-[11px] text-bimo-text/40">Depuis {{ $user->created_at->format('M Y') }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-5 py-3.5">
                        ...
                        <td class="px-5 py-3.5 font-body text-sm text-bimo-text/70">{{ $profil?->ville ?? '—' }}</td>
                        <td class="px-5 py-3.5 text-center">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-body font-medium
                                         {{ $user->biens_count > 0 ? 'bg-bimo-gold/10 border border-bimo-gold/20 text-bimo-gold' : 'bg-bimo-navy/10 border border-bimo-navy/15 text-bimo-text/50' }}">
                                {{ $user->biens_count > 0 ? $user->biens_count . ' bien(s)' : 'Aucun' }}
                            </span>
                        </td>
                        <td class="px-5 py-3.5">
                            @if($mode)
                            <div class="flex items-center gap-1.5 font-body text-sm text-bimo-text/70">
                        ...
                        <td class="px-5 py-3.5">
                            @if($profil?->ninea)
                            <span class="font-body text-[11px] bg-bimo-bg px-2 py-1 rounded-[5px] text-bimo-text/60">{{ $profil->ninea }}</span>
                            @else
                            <span class="font-body text-sm text-bimo-text/30">—</span>
                            @endif
                        </td>
                    ...
                    <tr>
                        <td colspan="7" class="px-5 py-16 text-center">
                            <div class="text-4xl mb-3">🏢</div>
                            <div class="font-display font-bold text-base text-bimo-text mb-2">Aucun propriétaire enregistré</div>
                            <p class="font-body text-sm text-bimo-text/50 mb-5">Commencez par ajouter le premier propriétaire.</p>
                            <a href="{{ route('admin.users.create', 'proprietaire') }}"
                               class="inline-flex items-center gap-2 px-5 py-2.5 bg-[var(--ac)] text-white font-display font-bold text-sm rounded-[10px] hover:opacity-90 transition-opacity duration-150">
                                + Ajouter un propriétaire
                            </a>
