...
<div class="space-y-4 md:space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <h1 class="font-display font-extrabold text-xl md:text-2xl text-bimo-navy tracking-tight leading-tight">Locataires</h1>
            <p class="font-body text-sm text-bimo-navy/50 mt-1">{{ $stats['total'] }} locataire(s) enregistré(s)</p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            <div class="font-body font-medium text-[9.5px] uppercase tracking-widest text-bimo-navy/50 mb-1">Avec contrat actif</div>
            <div class="font-display font-extrabold text-2xl text-bimo-navy leading-none">{{ $stats['actifs'] }}</div>
            <div class="font-body text-[10.5px] text-bimo-navy/40 mt-1.5">Loyer en cours</div>
        </div>
        <div class="bg-white rounded-[14px] border border-bimo-navy/10 p-4">
            ...
            <div class="font-body text-[10.5px] text-bimo-navy/40 mt-1.5">Locataires actifs</div>
        </div>
        <div class="bg-white rounded-[14px] border {{ $stats['sans_contrat'] > 0 ? 'border-bimo-red/20' : 'border-bimo-navy/10' }} p-4">
            <div class="font-body font-medium text-[9.5px] uppercase tracking-widest text-bimo-navy/50 mb-1">Sans contrat</div>
            <div class="font-display font-extrabold text-2xl {{ $stats['sans_contrat'] > 0 ? 'text-bimo-red' : 'text-bimo-navy' }} leading-none">{{ $stats['sans_contrat'] }}</div>
            <div class="font-body text-[10.5px] text-bimo-navy/40 mt-1.5">À reloger ou archiver</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[14px] border border-bimo-navy/10 overflow-hidden">
        <div class="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-bimo-navy/[5%] bg-bimo-bg2">
            <div class="flex flex-wrap items-center gap-2">
                <span class="font-display font-bold text-sm text-bimo-navy">Liste des locataires</span>
                {{-- Filtre tabs --}}
                <div class="flex gap-1 bg-bimo-bg border border-bimo-navy/10 p-1 rounded-[8px]">
                    ...
                    <button onclick="filterStatus('actif', this)" data-tab="actif"
                            class="filter-tab px-3 py-1 rounded-[6px] font-body font-medium text-xs transition-all duration-150 text-bimo-navy/50 hover:text-bimo-navy">
                        Avec contrat
                    </button>
        ...
        <div class="md:hidden divide-y divide-bimo-navy/[5%]">
            ...
            <div class="px-5 py-4" data-status="{{ $aContrat ? 'actif' : 'sans' }}" data-search="{{ strtolower($user->name . ' ' . $user->email . ' ' . $user->telephone) }}">
                <div class="flex items-center gap-3 mb-3">
                    ...
                    @else
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full border border-bimo-navy/15 bg-bimo-navy/5 text-[11px] font-body font-medium text-bimo-navy/50 flex-shrink-0">Sans contrat</span>
                    @endif
                </div>
                @if($contratActif)
                <div class="bg-bimo-bg border border-bimo-navy/10 rounded-[8px] px-3 py-2 mb-3">
                    <div class="font-body font-semibold text-xs text-bimo-navy">{{ $contratActif->bien->reference ?? '—' }}</div>
                    <div class="font-body text-[11px] text-bimo-navy/50">{{ $contratActif->bien->adresse ?? '' }}, {{ $contratActif->bien->ville ?? '' }}</div>
                ...
                <div class="flex items-center justify-end gap-1.5">
                    ...
                    <a href="{{ $waLink }}" target="_blank"
                       class="w-7 h-7 flex items-center justify-center border border-bimo-navy/10 rounded-[6px] text-bimo-navy/40 hover:border-[#25D366] hover:text-[#25D366] transition-all duration-150">
            ...
            <div class="px-5 py-16 text-center">
                ...
                <div class="font-display font-bold text-base text-bimo-navy mb-2">Aucun locataire enregistré</div>
                <p class="font-body text-sm text-bimo-navy/50 mb-5">Ajoutez votre premier locataire pour commencer.</p>
                <a href="{{ route('admin.users.create', 'locataire') }}"
            ...
            <table class="w-full text-sm" id="dt-loc">
                <thead>
                    <tr class="border-b border-bimo-navy/[5%] bg-bimo-bg">
                        ...
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/40">Situation</th>
                        <th class="px-5 py-3 text-left font-body font-medium text-[10px] uppercase tracking-widest text-bimo-navy/40">Contrat actif</th>
                ...
                <tbody class="divide-y divide-bimo-navy/[5%]">
                    ...
                    <tr class="hover:bg-bimo-bg transition-colors duration-100" data-status="{{ $aContrat ? 'actif' : 'sans' }}">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-2 mt-0.5">
                                        @if($estEntreprise)
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded-[4px] text-[9px] font-body font-bold bg-bimo-navy/10 text-bimo-navy/60">🏢 Entreprise</span>
                                        @endif
                                        <span class="font-body text-[10px] text-bimo-navy/40">Depuis {{ $user->created_at->format('M Y') }}</span>
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-5 py-3.5">
                            ...
                            <div class="font-body font-medium text-sm text-bimo-navy">{{ $profil->profession }}</div>
                            @endif
                            @if($profil?->employeur)
                            <div class="font-body text-xs text-bimo-navy/50">{{ $profil->employeur }}</div>
                            @endif
                        ...
                        <td class="px-5 py-3.5">
                            ...
                            <div class="bg-bimo-bg border border-bimo-navy/10 rounded-[8px] px-3 py-2">
                                <div class="font-body font-semibold text-xs text-bimo-navy">{{ $contratActif->bien->reference ?? '—' }}</div>
                                <div class="font-body text-[10px] text-bimo-navy/50 mt-0.5">{{ $contratActif->bien->adresse ?? '' }}, {{ $contratActif->bien->ville ?? '' }}</div>
                                <div class="font-display font-bold text-xs text-bimo-gold mt-0.5">{{ number_format($contratActif->loyer_contractuel, 0, ',', ' ') }} F/mois</div>
                            </div>
                            @else
                            <span class="font-body text-sm text-bimo-navy/30">Pas de contrat actif</span>
                            @endif
                        </td>
                        <td class="px-5 py-3.5 text-center">
                            ...
                            @else
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full border border-bimo-navy/15 bg-bimo-navy/5 text-[11px] font-body font-medium text-bimo-navy/50">Sans contrat</span>
                            @endif
                        </td>
                        <td class="px-5 py-3.5">
                    ...
                    <tr>
                        <td colspan="6" class="px-5 py-16 text-center">
                            <div class="text-4xl mb-3">👤</div>
                            <div class="font-display font-bold text-base text-bimo-navy mb-2">Aucun locataire enregistré</div>
                            <p class="font-body text-sm text-bimo-navy/50 mb-5">Ajoutez votre premier locataire pour commencer à gérer les locations.</p>
                            <a href="{{ route('admin.users.create', 'locataire') }}"
                               class="inline-flex items-center gap-2 px-5 py-2.5 bg-[var(--ac)] text-white font-display font-bold text-sm rounded-[10px] hover:opacity-90 transition-opacity duration-150">
                                + Ajouter un locataire
                            </a>
