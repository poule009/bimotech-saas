...
<head>
    ...
    <title>{{ config('app.name', 'BimoTech Immo') }}</title>
    ...
    {{-- Couleur dynamique de l'agence --}}
    @if(isset($currentAgency) && $currentAgency?->couleur_primaire)
    <style>
        :root {
            --agency: {{ $currentAgency->couleur_primaire }};
            --gold: {{ $currentAgency->couleur_primaire }};
            --gold-lt: color-mix(in srgb, {{ $currentAgency->couleur_primaire }} 12%, white);
            --gold-dk: color-mix(in srgb, {{ $currentAgency->couleur_primaire }} 70%, black);
        }
    </style>
    @endif
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <div class="sidebar-logo">
            @if(isset($currentAgency) && $currentAgency?->logo_path)
                <div class="logo-icon">
                    <img src="{{ Storage::url($currentAgency->logo_path) }}" alt="{{ $currentAgency->name }}">
                </div>
                <div>
                    <div class="logo-name">{{ $currentAgency->name }}</div>
                    <div class="logo-sub">Immo</div>
                </div>
            @else
                <div class="logo-icon">
                    <span>{{ strtoupper(substr($currentAgency->name ?? 'B', 0, 1)) }}</span>
                </div>
                <div>
                    <div class="logo-name">{{ $currentAgency->name ?? 'BimoTech' }}</div>
        ...
        <div class="sidebar-agency">
            ...
            <div class="agency-name">{{ $currentAgency->name }}</div>
        </div>
        @endif

        {{-- ALERTE ABONNEMENT --}}
        @if(isset($currentAgency) && $currentAgency->subscription && !$currentAgency->subscription->aAcces())
        <div class="sub-warning">
            ...
            Abonnement expiré
            <a href="{{ route('subscription.index') }}">Renouveler →</a>
        </div>
        @endif
        ...
        <nav class="sidebar-nav">
            ...
            @if(auth()->user()->isAdmin())
                ...
                <a href="{{ route('admin.paiements.index') }}" class="nav-item {{ request()->routeIs('admin.paiements.*') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>
                    Paiements
                </a>

                <a href="{{ route('admin.contrats.index') }}" class="nav-item {{ request()->routeIs('admin.contrats.*') ? 'active' : '' }}">
                ...
                <a href="{{ route('admin.impayes.index') }}" class="nav-item {{ request()->routeIs('admin.impayes.*') ? 'active' : '' }}">
                ...
                <a href="{{ route('admin.users.proprietaires') }}" class="nav-item {{ request()->routeIs('admin.users.proprietaires') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                    Propriétaires
                </a>

                <a href="{{ route('admin.users.locataires') }}" class="nav-item {{ request()->routeIs('admin.users.locataires') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>
                    Locataires
                </a>

                <a href="{{ route('admin.rapports.financier') }}" class="nav-item {{ request()->routeIs('admin.rapports.*') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
                    Rapports
                </a>

                <a href="{{ route('admin.agency.settings') }}" class="nav-item {{ request()->routeIs('admin.agency.*') ? 'active' : '' }}">
            ...
            @elseif(auth()->user()->isProprietaire())
                <a href="{{ route('proprietaire.dashboard') }}" class="nav-item {{ request()->routeIs('proprietaire.dashboard') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>
                    Mon tableau de bord
                </a>
                <a href="{{ route('proprietaire.paiements.pdf', '') }}" class="nav-item">
            ...
            @elseif(auth()->user()->isLocataire())
                <a href="{{ route('locataire.dashboard') }}" class="nav-item {{ request()->routeIs('locataire.dashboard') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>
                    Mon espace
                </a>
                <a href="{{ route('locataire.paiements') }}" class="nav-item {{ request()->routeIs('locataire.paiements') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>
                    Mes paiements
                </a>
            @endif
            ...
            <div style="position:relative">
                ...
                <div class="dropdown" id="dropdown">
                    <div style="padding:14px 16px;border-bottom:1px solid rgba(255,255,255,0.08)">
                        ...
                        <div style="font-size:13px;font-weight:500;color:rgba(255,255,255,0.8);margin-top:2px">{{ auth()->user()->email }}</div>
                    </div>
    ...
    <div class="main">
        ...
        <header class="topbar">
            ...
            <div class="topbar-right">
                <span class="top-date">{{ now()->translatedFormat('l d F Y') }}</span>
                <div class="top-notif">
