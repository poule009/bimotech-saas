@props([
    ...
    'compact' => false,        // version listes : padding réduit, sans icône
])
@php
    ...
    $pad        = $compact ? 'p-4' : 'p-5 lg:p-6';
    $valueSize  = $big ? 'text-3xl lg:text-[2.6rem]' : ($compact ? 'text-2xl' : 'text-2xl lg:text-3xl');

    $deltaUp = $delta !== null && $delta >= 0;
    $deltaPositive = ($deltaUp && $deltaGood) || (! $deltaUp && ! $deltaGood);
    $hasTop = isset($icon) || $delta !== null;
@endphp
<div {{ $attributes->merge(['class' => 'relative rounded-[14px] border transition-all duration-150 h-full '.$pad.' '.$variantClass]) }}>
    @if($href)
    ...
    <div class="flex items-start justify-between mb-4">
        @isset($icon)
        <div class="w-10 h-10 rounded-[10px] flex items-center justify-center {{ $iconWrap }}">{{ $icon }}</div>
        @else
        <span></span>
        @endisset
        @if($delta !== null)
