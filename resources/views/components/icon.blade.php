...
@php
    // Icônes au trait (style Feather/Lucide) — monochromes, prennent la couleur
    // du contexte via currentColor. Un seul endroit à maintenir.
    $icons = [
        ...
        'receipt'        => '<path d="M4 2v20l2-1 2 1 2-1 2 1 2-1 2 1 2-1 2 1V2l-2 1-2-1-2 1-2-1-2 1-2-1-2 1Z"/><path d="M8 7h8M8 11h6"/>',
        ...
        'sparkles'       => '<path d="M12 3l1.9 4.6L18.5 9.5 13.9 11.4 12 16l-1.9-4.6L5.5 9.5l4.6-1.9L12 3z"/><path d="M19 3v4M21 5h-4M5 17v2M6 18H4"/>',
    ];
    $path = $icons[$name] ?? '';
@endphp

<svg {{ $attributes->merge(['class' => 'inline-block shrink-0']) }} width="{{ $size }}" height="{{ $size }}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="{{ $stroke }}" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">{!! $path !!}</svg>
