...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> Tableau de Bord — <?php echo e($currentAgency?->name ?? 'BIMO-Tech'); ?> <?php $__env->endSlot(); ?>
    ...
    <div style="background: linear-gradient(145deg, #fffcf5, #f8f4e9); border: 1px solid rgba(181, 140, 90, 0.2); border-radius: 24px; padding: 32px; margin-bottom: 32px; box-shadow: 0 20px 40px -15px rgba(181, 140, 90, 0.1);">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:30px;">
            <div style="display:flex; align-items:center; gap:18px;">
                ...
                <div>
                    ...
                    <p style="font-size:13px; color:#b58c5a; font-weight:600;"><?php echo e($onboarding['nb_completes']); ?>/4 objectifs atteints</p>
                </div>
            </div>
            <div style="text-align:right;">
                <span style="font-size:28px; font-weight:900; color:#b58c5a; opacity:0.3;"><?php echo e(round(($onboarding['nb_completes'] / 4) * 100)); ?>%</span>
            </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(240px,1fr)); gap:20px;">
            ...
            <?php $__currentLoopData = $etapes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $e): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div style="background: <?php echo e($onboarding[$e['id']] ? 'rgba(255,255,255,0.8)' : '#fff'); ?>; border: 1px solid <?php echo e($onboarding[$e['id']] ? '#e2e8f0' : 'rgba(181, 140, 90, 0.2)'); ?>; border-radius:20px; padding:20px; position:relative; transition: all 0.3s ease;">
                    <div style="font-size:20px; margin-bottom:12px;"><?php echo e($e['icon']); ?></div>
                    <div style="font-size:14px; font-weight:700; color:#1a202c;"><?php echo e($e['titre']); ?></div>
                    
                    <?php if($onboarding[$e['id']]): ?>
                    ...
                    <?php else: ?>
                        <a href="<?php echo e(is_array($e['route']) ? route($e['route'][0], $e['route'][1]) : route($e['route'])); ?>" style="margin-top:10px; display:inline-block; font-size:12px; font-weight:800; color:#b58c5a; text-transform:uppercase; letter-spacing:1px;">Configurer →</a>
                    <?php endif; ?>
                </div>
    ...
    <div style="display:flex; gap:16px; margin-bottom:40px;">
        <a href="<?php echo e(route('admin.paiements.create')); ?>" class="btn btn-primary" style="background:#1a202c; color:white; border-radius:18px; padding:16px 28px; box-shadow: 0 10px 25px rgba(0,0,0,0.15);">
            <span style="font-size:18px; margin-right:8px;">+</span> Enregistrer un Paiement
        </a>
        <a href="<?php echo e(route('admin.contrats.create')); ?>" class="btn btn-secondary" style="background:white; color:#1a202c; border:1px solid #e2e8f0; border-radius:18px; padding:16px 28px;">
            Nouveau Contrat
        </a>
    </div>
    ...
    <div class="card" style="background: rgba(255, 255, 255, 0.5); backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.4); border-radius:32px; padding:32px; margin-bottom:40px;">
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:32px;">
            <div>
                ...
                <p style="font-size:13px; color:#718096; font-weight:500;">Bilan arrêté au <?php echo e(now()->translatedFormat('d F Y')); ?></p>
            </div>
            <a href="<?php echo e(route('admin.rapports.financier')); ?>" style="background:#f7f4ec; color:#b58c5a; padding:10px 20px; border-radius:12px; font-size:12px; font-weight:800; text-transform:uppercase;">Rapport détaillé</a>
        </div>
        ...
        <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:32px;">
            <div style="position:relative;">
                ...
                <div style="font-size:32px; font-weight:900; color:#1a202c;"><?php echo e(number_format($bilanMois['attendu'], 0, ',', ' ')); ?><span style="font-size:16px; margin-left:4px; opacity:0.4;">FCFA</span></div>
            </div>
            ...
            <div style="position:relative; padding-left:32px; border-left:1px solid #e2e8f0;">
                ...
                <div style="font-size:32px; font-weight:900; color:#16a34a;"><?php echo e(number_format($bilanMois['encaisse'], 0, ',', ' ')); ?><span style="font-size:16px; margin-left:4px; opacity:0.6;">FCFA</span></div>
                <div style="font-size:12px; font-weight:700; color:#16a34a; margin-top:4px;"><?php echo e($bilanMois['attendu'] > 0 ? round(($bilanMois['encaisse'] / $bilanMois['attendu']) * 100) : 0); ?>% de recouvrement</div>
            </div>

            <div style="position:relative; padding-left:32px; border-left:1px solid #e2e8f0;">
                <p style="font-size:11px; font-weight:800; color:<?php echo e($bilanMois['a_recouvrer'] > 0 ? '#dc2626' : '#16a34a'); ?>; text-transform:uppercase; letter-spacing:1.5px; margin-bottom:10px;">Reliquat</p>
                <div style="font-size:32px; font-weight:900; color:<?php echo e($bilanMois['a_recouvrer'] > 0 ? '#dc2626' : '#16a34a'); ?>;"><?php echo e(number_format($bilanMois['a_recouvrer'], 0, ',', ' ')); ?><span style="font-size:16px; margin-left:4px; opacity:0.4;">FCFA</span></div>
            </div>
        </div>
    </div>
    ...
    <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:20px; margin-bottom:40px;">
        ...
        <?php $__currentLoopData = $stats_cards; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="kpi" style="background:white; border:none; box-shadow: 0 10px 30px -10px rgba(0,0,0,0.05); padding:24px; border-radius:24px;">
                <div style="font-size:10px; font-weight:800; color:#a0aec0; text-transform:uppercase; letter-spacing:1px; margin-bottom:8px;"><?php echo e($sc['label']); ?></div>
                <div style="font-size:20px; font-weight:900; color:<?php echo e($sc['color']); ?>;"><?php echo e($sc['val']); ?></div>
                <div style="font-size:11px; color:#718096; margin-top:4px; font-weight:500;"><?php echo e($sc['sub']); ?></div>
            </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
    ...
    <div class="card" style="background:white; border-radius:28px; border:none; box-shadow: 0 10px 40px -15px rgba(0,0,0,0.05); overflow:hidden;">
        <div style="padding:24px 32px; border-bottom:1px solid #f7f4ec; display:flex; justify-content:space-between; align-items:center;">
            ...
            <a href="<?php echo e(route('admin.paiements.index')); ?>" style="font-size:13px; font-weight:700; color:#b58c5a;">Historique complet →</a>
        </div>
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $derniersPaiements; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr style="border-bottom:1px solid #f7f4ec;">
                            <td style="padding:20px 32px;"><span style="font-family:monospace; font-weight:700; color:#718096;">#<?php echo e($p->reference_paiement); ?></span></td>
                            <td style="padding:20px 16px;">
                                <div style="font-weight:700; color:#1a202c;"><?php echo e($p->contrat->locataire->name); ?></div>
                                <div style="font-size:12px; color:#a0aec0;"><?php echo e($p->contrat->bien->reference); ?></div>
                            </td>
                            <td style="padding:20px 16px;">
                                <span style="background:#f7f4ec; color:#b58c5a; padding:4px 10px; border-radius:8px; font-size:11px; font-weight:700;"><?php echo e(\Carbon\Carbon::parse($p->periode)->translatedFormat('M Y')); ?></span>
                            </td>
