...
class LocataireHandler extends AbstractImportHandler
{
    public function type(): string { return 'locataires'; }

    public function templateHeaders(): array
    {
        return ['nom', 'telephone', 'email', 'ville'];
    }

    public function templateSample(): array
    {
        return ['Moussa Fall', '76 000 00 02', '', 'Dakar'];
    }
    ...
    public function validate(array $row, int $line, array &$ctx): array
    {
        ...
        if ($normPhone && $this->telephoneExiste($normPhone, $ctx)) {
            return $this->doublon($line, "Téléphone déjà présent — locataire non réimporté.", $display);
        }
    ...
    {
        $code = $seq->formater('locataires', $sequence);
